 ...
 * --------------------------------------------------------------------------------------------------
 *
 * 运算符重载用于解决对象之间的运算问题。就是给已有的运算符赋予多重含义，使同一个运算符作用于不同类型的数据时产生不同的行为。
 * 目的是使得C++的运算符也能够用来操作对象。
 *
 * 允许重载大部分的内置运算符，但也有不可重载的运算符比如成员访问符. 、sizeof、::、#等。
 *
 * 运算符重载的实质是编写以运算符为名称的函数，使用运算符的表达式被解释为对重载函数的调用。函数名由关键字operator
 ...
 * 如果定义为全局函数，对于二元运算符，需要为函数传递俩个参数，即函数的参数个数就是运算符的操作数个数，运算符的操作数
 * 就称为函数的实参。
 * 如果定义为类的成员函数，对于二元运算符，则只需要传递一个参数，即函数的参数格式就是运算符的操作数个数减1，运算符的操作数有一个
 * 成为函数作用的对象，其余的成为成员函数的实参。
 * 声明为全局函数时，通常应是类的友元。
 *
 * 包含被重载的运算符的表达式会被编译乘对运算符函数的调用，运算符的操作数成为函数调用时的实参，运算的结果就是函数的返回值。
 * 运算符可以被多次重载，一般来说，倾向于将运算符重载为类的成员函数，这样能较好地体现运算符和类之间的关系。
 *
 *
 */
